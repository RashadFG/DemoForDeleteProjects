...
namespace Evimiz.Controllers
{
    public class ElanController : Controller
    {
        ...
        public IActionResult Haqqında(int? id)
        {
            ...
            {
                Advertisement =_context.Advertisements.Where(x=>x.IsConfirmed==true).FirstOrDefault(ad => ad.Id == id),
                Advertisements =_context.Advertisements.Where(x=>x.IsConfirmed==true).Where(ad => ad.CategoryId ==_context.Advertisements.Where(x=>x.IsConfirmed==true).FirstOrDefault(ada => ada.Id == id).CategoryId)
            };
